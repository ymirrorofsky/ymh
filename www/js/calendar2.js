var calUtil = {
	...
	eventName: "load",
	//初始化日历
	init: function(signList, s, token, success, fail) {
		// alert('init...')
		...
		calUtil.bindEnvent(signList, token, success, fail);
	},
	draw: function(signList) {
		...
		$("#calendar").html(str);
		//绑定日历表头
		var calendarName = calUtil.showYear + "年" + calUtil.showMonth + "月" + calUtil.showDate + '日';
		$(".calendar_month_span").html(calendarName);
	},
	//绑定事件
	bindEnvent: function(signList, token, success, fail) {
		// //绑定上个月事件
		...
		$(".calendar_record").click(function() {
			...
			if(date.getDate() == $(this).html()) {
				// return;
				$.ajax({
					//      	url:"http://ymh2.weishang6688.com/app/index.php?i=34&c=entry&m=welfare&do=users&op=sign&uid="+token.uid,
					url: "http://app.chinayiminhui.com/app/index.php?i=34&c=entry&m=welfare&do=users&op=sign&uid=" + token.uid,
					// url:"http://192.168.0.155/lemon/public/index.php/api/sign/index",   
					// data:{
					//   do:'sign',
					//       op:'index'
					// },
	...
	setMonthAndDay: function() {
		switch(calUtil.eventName) {
			case "load":
				...
				calUtil.showDate = current.getDate();
				break;
			case "prev":
